import { spotifyGet, spotifyPost, spotifyPut } from "../../utils/spotifyClient";
import type {
...
export const changePlaylistDetails = async (
  token: string,
  playlistId: string,
  body: PlaylistDetailsPayload
): Promise<void> => {
  return spotifyPut<void>(`/playlists/${playlistId}`, token, body);
};
